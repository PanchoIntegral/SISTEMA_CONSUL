<template>
    <div class="border rounded-md shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden bg-white group"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          <div class="space-y-1.5">
            ...
            <div v-if="appointment.notes" class="flex mt-1.5">
              <div class="w-7 h-7 bg-gray-100 rounded-full flex-shrink-0 flex items-center justify-center mr-1.5">
          ...
          <div class="space-y-1.5 border-t pt-2 md:pt-0 md:border-t-0 md:border-l md:pl-3 mt-1.5 md:mt-0">
            ...
            <div v-if="appointment.status === 'En Consulta'" class="space-y-1.5">
              <div class="bg-wave-blue bg-opacity-5 rounded-md p-1.5 flex items-center justify-between">
              ...
              <div class="bg-wave-teal bg-opacity-5 rounded-md p-1.5 flex items-center justify-between">
            ...
            <div v-if="appointment.status === 'Completada' && (appointment.calculated_wait_time_seconds !== null || appointment.calculated_consultation_time_seconds !== null)" 
                class="flex gap-1.5">
              <div class="flex-1 bg-wave-blue bg-opacity-5 rounded-md p-1.5 text-center">
                <span class="text-xs text-wave-blue font-medium block">Espera</span>
                <span class="text-xs font-semibold text-navy">{{ formatDuration(appointment.calculated_wait_time_seconds) }}</span>
              </div>
              <div class="flex-1 bg-wave-green bg-opacity-5 rounded-md p-1.5 text-center">
                <span class="text-xs text-wave-green font-medium block">Consulta</span>
                <span class="text-xs font-semibold text-navy">{{ formatDuration(appointment.calculated_consultation_time_seconds) }}</span>
              </div>
            </div>
      ...
      <!-- Botones de acción -->
      <div class="px-3 py-2 bg-gray-50 border-t flex flex-wrap gap-1.5 justify-end">
        <!-- Botones de estado -->
        <button
        ...
        <button
          ...
          class="text-xs bg-wave-green bg-opacity-10 text-wave-green px-2.5 py-1 rounded-md hover:bg-opacity-20 focus:outline-none focus:ring-2 focus:ring-wave-green focus:ring-offset-1 transition-colors"
        >
          <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 inline-block mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
          </svg>
